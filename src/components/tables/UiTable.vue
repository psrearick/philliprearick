<template>
    <div :class="(wide ? 'lg:-mx-24' : '') + ' ui-table overflow-auto max-h-screen'">
        <table class="text-xs text-center overflow-x-scroll table-auto w-full">
            <thead class="sticky top-0 z-10">
                <tr v-for="(row, index) in data.headers" :key="index">
                    <th v-for="(cell, cellIndex) in row" :key="cellIndex" :class="'font-bold bg-gray-200 ' + stickyCol(cellIndex)">{{ cell }}</th>
                </tr>
            </thead>
            <tbody v-if="data.data">
                <tr v-for="(row, index) in data.data" :key="index">
                    <td v-for="(cell, cellIndex) in row" :key="cellIndex" :class="boldCell(cellIndex) + ' bg-gray-100 ' + stickyCol(cellIndex)">{{ cell }}</td>
                </tr>
            </tbody>
